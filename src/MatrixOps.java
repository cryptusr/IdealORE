...
public class MatrixOps{
	...
	public static Element[][] generatePermutationMatrix(int dimension) {
        if (dimension <= 0) {
            throw new IllegalArgumentException("Dimension must be a positive integer.");
        }

        // Initialize an array for the permutation
        ...
        Element[][] matrix = new Element[dimension][dimension];
        for (int i = 0; i < dimension; i++) {
        	for (int j = 0; j < dimension; j++)
        		matrix[i][j] = pairing.getZr().newZeroElement();
        }
        
        for (int i = 0; i < dimension; i++) {
            matrix[i][permutation[i]] = pairing.getZr().newOneElement();
        }
	...
	public static Element[][] identityMatrix(int size) {
	    Element[][] identity = new Element[size][size];

	    // Initialize the matrix with zero elements
	    for (int i = 0; i < size; i++) {
	        for (int j = 0; j < size; j++) {
	            identity[i][j] = pairing.getZr().newZeroElement();
	        }
	    }

	    // Set diagonal elements to one
	    for (int i = 0; i < size; i++) {
	        identity[i][i].set(pairing.getZr().newOneElement());
	    }
